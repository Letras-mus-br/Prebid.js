...
import {
  ...
  trimBid,
  trimBidderRequest,
} from "../libraries/letras/letrasUtils.js";

/**
 * letrasAnalyticsAdapter.js - analytics adapter for Letras
 */
export const storage = getStorageManager({
  moduleType: MODULE_TYPE_ANALYTICS,
  moduleName: "letrasAnalytics",
});
const DEFAULT_EVENT_URL = "https://pbs.sscdn.co/events";
const analyticsType = "endpoint";
const analyticsName = "Letras Analytics";
const _VERSION = 1;
...
let flushInterval;
let eventQueue = [];
let letrasAnalyticsEnabled = false;

const letrasAnalytics = Object.assign(
...
letrasAnalytics.originEnableAnalytics = letrasAnalytics.enableAnalytics;
letrasAnalytics.enableAnalytics = function (config) {
  ...
  if (Math.floor(Math.random() * sampling) === 0) {
    letrasAnalyticsEnabled = true;
    flushInterval = setInterval(flush, 1000);
  } else {
    logInfo(`${analyticsName} isn't enabled because of sampling`);
  }

  letrasAnalytics.originEnableAnalytics(config);
};

letrasAnalytics.originDisableAnalytics = letrasAnalytics.disableAnalytics;
letrasAnalytics.disableAnalytics = function () {
  if (!letrasAnalyticsEnabled) {
    return;
  }
  flush();
  clearInterval(flushInterval);
  letrasAnalytics.originDisableAnalytics();
};
...
function handleEvent(eventType, eventArgs) {
  ...
  if (eventArgs) {
    eventArgs = hasNonSerializableProperty(eventArgs)
      ? eventArgs
      : deepClone(eventArgs);
  } else {
    eventArgs = {};
  ...
  switch (eventType) {
    case EVENTS.AUCTION_INIT: {
      ...
      pmEvent.bidderRequests =
        eventArgs.bidderRequests &&
        eventArgs.bidderRequests.map(trimBidderRequest);
    ...
    case EVENTS.AUCTION_END: {
      ...
      pmEvent.bidsReceived =
        eventArgs.bidsReceived && eventArgs.bidsReceived.map(trimBid);
    ...
    case EVENTS.BID_TIMEOUT: {
      pmEvent.bidders =
        eventArgs && eventArgs.map ? eventArgs.map(trimBid) : eventArgs;
...
adapterManager.registerAnalyticsAdapter({
  adapter: letrasAnalytics,
  code: "letrasAnalytics",
});

letrasAnalytics.getOptions = function () {
  return initOptions;
};

letrasAnalytics.flush = flush;

export default letrasAnalytics;
